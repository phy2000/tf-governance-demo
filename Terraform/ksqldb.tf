...
resource "confluent_role_binding" "app-ksql-all-topic" {
  ...
  crn_pattern = "${confluent_kafka_cluster.demo-cluster.rbac_crn}/kafka=${confluent_kafka_cluster.demo-cluster.id}/topic=*"
}

resource "confluent_role_binding" "app-ksql-all-group" {
  principal   = "User:${confluent_service_account.app-ksql.id}"
  role_name   = "ResourceOwner"
  crn_pattern = "${confluent_kafka_cluster.demo-cluster.rbac_crn}/kafka=${confluent_kafka_cluster.demo-cluster.id}/group=*"
}

resource "confluent_role_binding" "app-ksql-all-transactions" {
  principal   = "User:${confluent_service_account.app-ksql.id}"
  role_name   = "ResourceOwner"
  crn_pattern = "${confluent_kafka_cluster.demo-cluster.rbac_crn}/kafka=${confluent_kafka_cluster.demo-cluster.id}/transactional-id=*"
}
...
resource "confluent_role_binding" "app-ksql-schema-registry-resource-owner" {
  principal   = "User:${confluent_service_account.app-ksql.id}"
  role_name   = "ResourceOwner"
  crn_pattern = format("%s/%s", data.confluent_schema_registry_cluster.demo-schema.resource_name, "subject=*")
}
...
resource "confluent_ksql_cluster" "main" {
  ...
  depends_on = [
    ...
    //confluent_schema_registry_cluster.demo-schema
  ]
}
